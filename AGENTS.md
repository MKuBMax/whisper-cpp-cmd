...
- 系统音频参考消除的关键方案已真机验收：常驻 sysref + AudioBufferList 第一声道 + STFT 维纳 + 频点相干压残差。改前先读扬声器串扰专题；不能改回按键冷启动、能量门控、单增益时域相减、整帧 NLP 或能量双讲检测，也不能先峰值归一化再消除。
- 录音浮窗的系统材质已经在真实 app 进程中验证不可用；当前方案是纯 alpha 半透明皮肤，若再改外观必须在真实 app 中验收。

## 开发与部署工作流

- **默认处于开发模式。** 修改代码、运行测试、调试和让用户试用时，只使用 `bash run_dev_app.sh`（DEV App，alias 引用源码）；不得调用 `ship_app.sh`、`build_app.sh`、`package_app.sh` 或替换 `/Applications/WhisperCppCmd.app`。裸 Python 启动（`run_dev.sh`）已下线，测试仍用 `.venv-arm64/bin/python -m pytest tests/`。
...
- `run_dev_app.sh` 构建 DEV App 并启动（alias 引用源码，改完即生效），正式版与 DEV 同时只跑一个，启动前经 `scripts/single_instance.sh` 互斥；`ship_app.sh` 是发布动作，不是普通重启或审核动作。
