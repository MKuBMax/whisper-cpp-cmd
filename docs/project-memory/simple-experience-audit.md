...
- 2026-09-05：设置页补全输入监控权限状态和授权入口，打开窗口时重新检查权限；输入监控未授权时显示快捷键风险提示。辅助功能授权下仍允许尝试监听，不把线程存活视为物理按键已验证。
...
- 菜单栏状态项日志显示 `visible=True`、`button_hidden=False`、按钮宽度 32 pt。状态栏窗口的内部 frame 不能证明图标在屏幕上，菜单栏图标仍需在用户自己的菜单栏环境中亲眼确认。
- 2026-09-05 通过系统设置和只读配置进一步确认：macOS「菜单栏项目」开关可以单独关闭 WhisperCppCmd；Thaw 的 `MenuBarItemManager.savedSectionOrder.hidden` 也保留了 `com.mkbm.whispercppcmd:Item-0` 和三个旧状态项标识。Thaw 是按保存的布局隐藏项目，不代表它发生故障；App 代码没有调用 `setVisible_(False)`，只能请求系统显示。
- 当前测试环境的麦克风和辅助功能已允许，日志确认全局热键监听器已启动；右 Command 录音、真实光标输入和剪贴板回退仍需分别实测，不能由权限状态替代。
